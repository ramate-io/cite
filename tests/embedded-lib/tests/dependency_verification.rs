...
fn test_cite_dependencies_embedded_lib() {
    ...
    assert!(
        runtime_deps.len() >= 3,
        "Expected at least 3 runtime dependencies (cite + embedded-hal + nb), found {}",
        runtime_deps.len()
    );
    ...
    assert!(dep_names.contains("nb"), "Missing nb dependency");
}
...
fn test_embedded_hal_integration() {
    ...
    use cite_embedded_lib_test::{EmbeddedDriver, TemperatureSensor, SensorReading, config::SystemConfig};
