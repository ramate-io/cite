...
#[cite(mock, same = "no-std binary lib function")]
pub fn binary_lib_function() {
    // This function should compile in a no-std environment
}

#[cite(mock, same = "no-std binary struct")]
pub struct BinaryStruct {
    pub value: u32,
}

#[cite(mock, same = "no-std binary enum")]
pub enum BinaryEnum {
    Option1,
    Option2(u32),
}
...
impl BinaryStruct {
    #[cite(mock, changed = ("old constructor", "new constructor"), level = "SILENT")]
    pub fn new(value: u32) -> Self {
        Self { value }
    }
    
    #[cite(mock, same = "getter method")]
