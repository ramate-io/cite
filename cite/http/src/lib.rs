		...
		match self {
			MatchExpression::Regex(pattern) => {
				let regex = Regex::new(pattern).map_err(|e| {
					SourceError::ContentParsing(format!(
			...
			MatchExpression::XPath(_expression) => {
				...
				Err(SourceError::ContentParsing(
					"XPath expressions are not yet implemented".to_string(),
				))
			}
...
impl SourceUrl {
	...
	pub fn new(url: &str) -> Result<Self, SourceError> {
		...
		Ok(Self { url: url.to_string(), normalized, fragment })
	}
...
impl HttpMatch {
	...
	pub fn with_match_expression(
		url: &str,
		expression: MatchExpression,
	) -> Result<Self, SourceError> {
		Self::with_match_expression_and_cache_behavior(
			url,
			expression,
	...
	pub fn with_match_expression_and_cache_behavior(
		...
		let cache_path = format!(
			"http_{}_{}",
			Self::url_to_cache_key(url),
			Self::match_expression_to_cache_key(&expression)
		);
		...
		Ok(Self { matches: expression, source_url, cache_path, id, cache, cache_behavior })
	}
...
fn determine_cache_behavior_for_macro(
	cache_override: Option<cite_cache::CacheBehavior>,
) -> cite_cache::CacheBehavior {
...
mod tests {
	...
	#[test]
	fn test_match_expression_regex() -> Result<(), anyhow::Error> {
	...
	#[test]
	fn test_source_url_validation() -> Result<(), anyhow::Error> {
	...
	#[test]
	fn test_http_match_creation() -> Result<(), anyhow::Error> {
	...
	#[test]
	fn test_http_source_fetch() -> Result<(), anyhow::Error> {
	...
	#[test]
	fn test_http_diff() -> Result<(), anyhow::Error> {
		let referenced = ReferencedHttp {
	...
	#[test]
	fn test_cacheable_serialization() -> Result<(), anyhow::Error> {
		let referenced = ReferencedHttp {
	...
	#[test]
	fn test_regex_match_expression() -> Result<(), anyhow::Error> {
	...
	#[test]
	fn test_css_selector_match_expression() -> Result<(), anyhow::Error> {
	...
	fn test_timestamp_endpoint_shows_diff() -> Result<(), anyhow::Error> {
		...
		let unified_diff = diff.unified_diff().ok_or(anyhow::anyhow!("No unified diff"))?;
	...
	#[test]
	fn test_http_diff_formatting() -> Result<(), anyhow::Error> {
		let referenced = ReferencedHttp {
			content: "Line 1\nOld Line 2\nLine 3".to_string(),
			metadata: HashMap::new(),
			source_url: SourceUrl::new("https://example.com")?,
	...
	#[test]
	fn test_url_fragment_parsing() -> Result<(), anyhow::Error> {
	...
	#[test]
	fn test_fragment_match_expression() -> Result<(), anyhow::Error> {
		let html_content = r#"
            <html>
                <body>
                    <h1>Main Title</h1>
	...
	#[test]
	fn test_auto_fragment_detection() -> Result<(), anyhow::Error> {
	...
	#[test]
	fn test_fragment_based_content_extraction() -> Result<(), anyhow::Error> {
	...
	#[test]
	fn test_cache_hits_with_static_content() -> Result<(), anyhow::Error> {
	...
	#[test]
	fn test_cache_key_includes_match_expression() -> Result<(), anyhow::Error> {
		...
		assert_ne!(
			css_id, full_id,
			"CSS selector and full document should have different cache IDs"
		);
